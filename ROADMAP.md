...
| Distribution `hal.10` | `hal.10-alpha.38`: candidat correctif identifie et deploiement ferme |
...
Etat source `hal.10-alpha.37`: toute promotion reconstruit proprement le
...
La publication canari refuse aussi un worktree sale, une endurance incomplete,
une version divergente, un SHA-256 ou MD5 different et tout chemin hors du canal
prive `development`. Elle publie le firmware avant son manifeste et ne modifie
jamais le pointeur racine `hal.6`.
L'installation reste une operation distincte: elle repete le preflight, exige
une confirmation exacte, transmet le binaire revu directement a l'implementation
`espota2` epinglee, puis exige via l'API chiffree la version attendue, l'etat
`waiting`, la sante `healthy` et aucune erreur vocale. Le rollback `hal.6` a sa
propre confirmation, lie SHA-256 et MD5 au manifeste immuable, et n'est jamais
declenche automatiquement.
Les sept portes canari fondamentales partagent obligatoirement un dossier
`canary-core` unique, lie au commit, a l'appareil, a la version et au SHA-256
OTA. Il impose une OTA exacte, dix redemarrages firmware, dix coupures Wi-Fi
ciblees, dix redemarrages HA, la persistance/nettoyage confidentialite, cent TTS
entendus et l'artefact `hal.6` conserve. Chaque resultat lie aussi son journal
brut par SHA-256; dossiers divises, chemins non surs, seuils atteints ou depasses,
interventions manuelles et etat final non sain sont refuses.
Le scelleur canari refuse un worktree sale ou un fichier final existant, repete
la porte endurance/candidat, valide l'artefact prive `hal.6`, recalcule toutes
les identites et empreintes et ignore l'identite non fiable du brouillon. Il
ecrit d'abord un fichier temporaire synchronise, valide le dossier complet puis
le publie atomiquement sans ecrasement; tout echec laisse son dossier final absent.
Les quatre portes source beta partagent un dossier hashe: deux builds propres
identiques, le run CI du commit exact, la taille recalculee et un audit des
fichiers suivis comparant au moins trois vrais secrets sans jamais les imprimer.
Stable lie aussi sa cible 93% au meme dossier. Journaux modifies, dossiers
divises, build flottant, mauvais commit, depassement ou secret exemple sont
refuses. Les secrets actuellement reconnaissables comme fixtures devront etre
rotes apres l'endurance et avant toute qualification beta.
Le dossier source est maintenant derive sans metriques libres depuis le commit
...
Le fichier prive audite est resolu une fois puis sur-monte en lecture seule sur
`/config/secrets.yaml` dans chacun des deux conteneurs. Le JSON CI doit aussi
prouver le commit propre, l'unique job `compile`, son succes et son URL canonique
avant le premier build; auditer un secret et en compiler un autre est impossible.
La preuve source v2 lie maintenant l'empreinte du fichier prive aux deux journaux
...
sans conserver ni reactiver le mot de passe retire.
...
fichiers `0600`. Aucune connexion, compilation ou installation n'est effectuee;
une erreur supprime toute sortie partielle sans toucher une destination existante.
Elle exige maintenant avant toute generation le resume 24 heures exact, recent
et reussi pour la meme version. Son manifeste v2 lie SHA-256, heure de fin et
version; l'installateur exige a nouveau ce meme resume avant et apres confirmation.
La transition exige ensuite bundle et audit fermes, endurance et artefact exacts
et une confirmation dediee. L'ancien OTA ne sert qu'a l'upload; la nouvelle cle
API prouve le boot exact et sain avant remplacement atomique du fichier actif.
L'ancien OTA est alors retire. Une interruption est reprenable, mais un jeu
actif partiel ou etranger est refuse sans modification.
