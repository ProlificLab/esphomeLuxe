...
class Hal6UsbRecoveryTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.factory.write_bytes(
            b"\xe9" + b"\0" * (checker.APP_OFFSET - 1) + self.ota.read_bytes()
        )
    ...
    def validate(self) -> dict[str, object]:
        with patch.object(
            ...
            return checker.validate_usb_recovery(
                self.ota,
                self.factory,
                self.manifest,
    ...
    def test_factory_digest_mismatch_fails(self) -> None:
        with patch.object(
            ...
            checker.validate_usb_recovery(
                self.ota, self.factory, self.manifest, "0" * 64
            )

    def test_wrong_offset_or_trailing_data_fails(self) -> None:
        for data in (
            b"\xe9" + b"\0" * (checker.APP_OFFSET - 2) + self.ota.read_bytes(),
