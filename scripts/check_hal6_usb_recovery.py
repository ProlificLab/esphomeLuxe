...
def validate_usb_recovery(
    ...
    if re.fullmatch(r"[0-9a-f]{64}", expected_factory_sha256) is None:
        raise RuntimeError("Factory SHA-256 must be an exact lowercase digest")
    ...
    if factory_sha256 != expected_factory_sha256:
        raise RuntimeError("Factory image differs from the reviewed SHA-256")
    if len(factory) != APP_OFFSET + len(ota):
        raise RuntimeError("Factory image has an unexpected layout or trailing data")
    if factory[:1] != b"\xe9" or ota[:1] != b"\xe9":
        raise RuntimeError("Factory or OTA image lacks the ESP32 image marker")
...
def main() -> None:
    ...
    result = validate_usb_recovery(
        args.ota, args.factory, args.manifest, args.expected_factory_sha256
    )
